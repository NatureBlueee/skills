...
- **Existing-build detection (migration)**: call `query_toolkit` query_type='migration_preflight' (takes `account` only; the testnet→mainnet direction is implicit and is echoed back as `source_network` / `target_network`). If the account already built on testnet, switch to the **mainnet customization guide** — re-confirm payment token / location / arbitration (the checklist is returned by the same query), rather than re-asking everything.
...
- **MCP**: `wowok_buildin_info` info='funding guidance' (testnet=WOW) + info='mainnet bridge tokens' (USDT/USDC/ETH/BTC/WOW type tags).
